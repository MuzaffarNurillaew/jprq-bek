...
KIND_CLUSTER ?= jprq-bek-test-e2e
...
.PHONY: build-cli
build-cli: fmt vet ## Build agent (cli) binary.
...
.PHONY: run-cli
run-cli: fmt vet ## Run the agent (cli) from your host.
	go run ./cli
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name jprq-bek-builder
	$(CONTAINER_TOOL) buildx use jprq-bek-builder
	...
	- $(CONTAINER_TOOL) buildx rm jprq-bek-builder
	rm Dockerfile.cross
