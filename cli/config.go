// Copied from github.com/azimjohn/jprq at cli/config.go, upstream commit
// 3c10e25, licensed under the MIT License (Copyright (c) 2020 Azimjon
// Pulatov). The token-file-with-fallback patch is called out inline
// below; see docs/DESIGN.md §7.1 and §8.1.
package main
...
func (c *Config) Load() error {
	// JPRQ_TOKEN_FILE (DESIGN.md §7.1, §8.1): read the raw token from a
	// mounted Secret file if set, trimming trailing whitespace. Falls
	// back to upstream's local config file below when unset, so this
	// stays a drop-in CLI on a laptop (DESIGN.md §4.1).
	if tokenFile := os.Getenv("JPRQ_TOKEN_FILE"); tokenFile != "" {
		data, err := os.ReadFile(tokenFile)
		if err != nil {
			return fmt.Errorf("error reading %s: %s", tokenFile, err)
		}
		c.Local.AuthToken = strings.TrimSpace(string(data))
	} else {
